...
class Group:
    ...
    def add_user(self,email,name,uid):
        flag = 0
        
        if len(self.user_array) > 0:
            for user in self.user_array:
                if user.email == email:
                    flag = 1
        if flag == 0:
            x = User(email,name,uid)
            self.user_array.append(x)
    
    def add_vm(self,email,uid,vmid,flavor):
        if len(self.user_array) > 0:
            for user in self.user_array:
                if user.email == email:
                    user.add_vm(vmid,flavor)
...
class User:
    def __init__(self,email,name,uid):
        ...
        self.uid = uid
        self.vm_array = []
    def add_vm(self,vmid,flavor):
        x = VM(vmid,flavor) 
        self.vm_array.append(x)
    def print_this(self):
        print "Name: " + self.name + " Email: " + self.email + " UID: " + self.uid
        for vm in self.vm_array:
            vm.print_this()


class VM:
    def __init__(self,vmid, flavor):
        self.vmid = vmid
        self.flavor = flavor
    def print_this(self):
        print "VM_ID: " + str(self.vmid) + "VM_Flavour: " + str(self.flavor)
...
def ceilometer_client():
    ...
    url = os.environ.get('OS_AUTH_URL', None)
...
def server_metrics(serverid):
    query = [dict(field='resource_id', op='eq', value=serverid)]
...
#    for host in hosts:
    ...
    c = 1
    print "USER_ID - VM_ID - CORES - STATE - EMAIL: "
        ...
        for instance in vms_on_host:
          ...
          #  IMAGE = str(temp['id']);
            ...
            if (NCPU == 8 or NCPU == 16):
                    ...
                    #    print server_metrics(UID) 
                cmd = "keystone user-get " + USER_ID + " | grep email | cut -f3 -d'|' | tr -d ' ' " 
                #"cut -f3 -d':' | tr -d ' ' | tr -d '\|' " 
                email = cmdline(cmd) 
                if "unimelb.edu.au" in email: 
                        # print USER_ID + " " + UID + " " + str(NCPU) + " " + STATE + " " + email
                    group.add_user(email,email,UID)
                    group.add_vm(email,USER_ID,UID,NCPU)
                  #  print "okay"
            #        stats = server_metrics(UID)
            #        ceilometer_data_age = stats.get('duration_end', '')
            #        if ceilometer_data_age:
            #            ceilometer_data_age = (datetime.now() - parse_date(ceilometer_data_age))
             #           if ceilometer_data_age.days == 0:
                  #  print "CPU AVG: " + str(stats.get('avg', '')) + " CPU MAX: " + str(stats.get('max','')) + " CPU MIN: " + str(stats.get('min',''))
